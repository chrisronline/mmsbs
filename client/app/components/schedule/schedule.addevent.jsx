...
var CalendarAddEventFormComponent = React.createClass({
  ...
  getInitialState: function() {
    ...
    return {
      event: {
        ...
        name: 'Chris',
        phone: '585-236-3326',
        type: _.find(config.types, { name: 'shave' }),
  ...
  _handleTypeChange: function(e, key) {
    var event = _.extend(this.state.event, {
      type: _.find(config.types, { name: e.target.value })
    });
  ...
  render: function() {
    var hours = config.schedule.hours;
    var dateMoment = moment(this.state.event.date, DATE_INPUT_FORMAT);
    // TODO: all these moment calls might be expensive
    var dateHours = hours[dateMoment.format('d')];
    var timeBlockStart = dateMoment.clone().hour(dateHours.open);
    var timeBlockEnd = dateMoment.clone().hour(dateHours.close);
          ...
          <div className="panel panel-default">
            <form className="calendar-add-event-form panel-body form-horizontal">
              ...
              <FormInputComponents.Time id="inputTime" label="Time" type="time" placeholder="Time"
                ...
                timeBlockIncrement={this.state.event.type.timeBlockInMinutes}
                timeBlockStart={timeBlockStart} timeBlockEnd={timeBlockEnd}/>
              ...
              <FormInputComponents.Select id="inputType" label="Type"
                defaultValue={this.state.event.type.name} onChange={_.partialRight(this._handleTypeChange, 'type')}
                options={config.types} optionValue="name" optionLabel="label"/>
